...
Maybe mention that **understanding biological interactions from an activate/inhibit point of view is more intuitive/easy**.
...
.center[![:scale 55%](img/trial-lecture/eduati_parameters.jpg)]]
...
- Interactions .red[missing or incorrect]
]]
...
### Mechanistic models vs standard Machine Learning methods
...
- ML methods .red[`r shiny::icon("times")`] .small.purple[Julkunen (2020)]
- ODE models .green[`r shiny::icon("check")`] .small.purple[Fröhlich (2018)]
...
.pull-left-67[
### Visual Neural Network (VNN)

![:scale 85%](img/trial-lecture/kuenzi_drugcell.jpg)
]

.pull-right-33[
### DrugCell: Interpretable deep learning model of human cancer cells

- .green[Learns] biological mechanisms
- .green[Stratifies] clinical outcomes
- .green[Designs] synergistic drug combinations

.small.purple[Kuenzi (2020). Predicting Drug Response and Synergy Using a Deep Learning Model of Human Cancer Cells. Cancer Cell, 38(5), 672–684. https://doi.org/10.1016/J.CCELL.2020.09.014]
]

???

### New trend in AI research (briefly mention? nah!)

Deep NNs have almost exhausted their utility (recognize objects, self-driving cars, beating world-champion at chess and Go), need for fresh approaches, going back to the roots (studying how babies learn, what is intelligence, how do we achieve generalization - take what you learn and use it in new situations, common sense, etc.). 

Towards this trend are **efforts that make existing methods more intepretable**, e.g. why Deep NNs make the decisions that they make.

### DrugCell

- Genotype + drug structure predicts response to therapy
- Model Architecture (.green[green image]):
Form top to bottom, increase in scale and complexity: **Genes, proteins, complexes, signaling pathways, organelles, cellular processes!**
- Since DrugCell’s VNN is structured according to the hierarchy of biological subsystems comprising a human cell, its output (genotype embedding) is the result of state changes in particular subsystems within that hierarchy. .green[To identify the most important of these subsystems], **we scored subsystems** by the degree to which their states were significantly more predictive of a drug response than the states of their child subsystems using the relative local improvement in predictive power metric (RLIPP)
- Authors analyze the top 5 and bottom 5 DrugCell subsystems for each of the drugs tested **to nominate synergistic and non-synergistic drug combinations**.
...
.pull-left-73[
`r shiny::icon("long-arrow-alt-right")` .bold.green[Hypothesis]: Cancer exploits cross-talk signaling to develop resistance
...
- Build network
- Compute PCI (\*)
- .little[PCI `r shiny::icon("arrow-up")` = synergy `r shiny::icon("arrow-up")`]
...
- **Cancer is never a single-cell disease!** 
  - There are Various types of cells in the tumor-microenvironment
- General categorization
  - **lattice-based** (track cells in a fixed grid, CA as an example)
  - **lattice-free** (ABM as an example)
...
- Intuitive for multi-cellular systems
...
.pull-right[
![](img/trial-lecture/agent_based_modeling.jpg)

.small.purple[ElKalaawy & Wassal (2015). Methodologies for the modeling and simulation of biochemical networks, illustrated for signal transduction pathways: A primer.]
]
...
---
.pull-left-67[
.bold[Simulation of tumor growth]

![](img/trial-lecture/physicell_tumor_growth.png)

.bold[Simulation of cancer immunotherapy in 3D]
...
.littler[
- Clonal expansion
- Mechanics of cell proliferation
- Biased stochastic immune cell migration leads to insufficient cancer killing
]

.red.bold[Drawbacks]
.littler[
- 2 weeks of computations
- Simplifications
]

.smaller.purple[Ghaffarizadeh (2018). PhysiCell: An open source physics-based cell simulator for 3-D multicellular systems]

.small[`r shiny::icon("long-arrow-alt-right")` Parameter optimization in 2D simulations correlated with patient response to immunotherapy]

.smaller.purple[Ozik (2019). Learning-accelerated discovery of immune-tumour interactions]
]
...
Knowledge of assets and drawbacks .green[`r shiny::icon('smile')`]
...
- .purple[Wooldridge, M. (2002)]. An Introduction to Multiagent Systems. John Wiley & Sons
...
- .purple[ElKalaawy & Wassal (2015)]. Methodologies for the modeling and simulation of biochemical networks, illustrated for signal transduction pathways: A primer. Biosystems, 129, 1–18. https://doi.org/10.1016/J.BIOSYSTEMS.2015.01.008
...
<br/>&nbsp;<br/>
.little[.bold[Code for slides]: https://github.com/bblodfon/r-pres/blob/master/phd_trial_lecture.Rmd]
