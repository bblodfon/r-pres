...
.little[
Automated pipeline **performs on par** with curated model from .purple[Flobak (2015)]
...
.little[
**Reduce drug screen** to .green[24%]
  - 36 out of 153 combos
  - .red[Miss 2 observed synergies]
  
.small.purple[Flobak (2019). A high-throughput drug combination screen of targeted small molecule inhibitors in cancer cell lines]
]]
...
.pull-left[![](img/thesis/td-link-operator.svg)]
.pull-right[![](img/thesis/td-thres.svg)]
...
.bold[Expected behavior for large number of regulators:]

`r shiny::icon("long-arrow-alt-right")` $TD_{AND-NOT} = \frac{1}{2^k}-\frac{1}{2^n} \sim \frac{1}{2^k} \xrightarrow{k \rightarrow \infty} 0$ (depends on #.red[inhibitors])

`r shiny::icon("long-arrow-alt-right")` $TD_{OR-NOT} = 1 - \frac{1}{2^m} + \frac{1}{2^n} \sim 1-\frac{1}{2^m} \xrightarrow{m \rightarrow \infty} 1$ (depends on #.green[activators])
