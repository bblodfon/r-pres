...
![](img/trial-lecture/ubds_image.png)
]
...
- Model identified *synergistic* effect of joint `Pi3k` and `Tak1` inhibition
...
.pull-left-58[![](img/thesis/interpretability.svg)
...
- **Link operator functions** with a total of $n = m + k$ regulators:
  ...
  1. $f_{AND-NOT}(x,y) = \left(\bigvee_{i=1}^{m} x_i\right) \land \lnot \left(\bigvee_{j=1}^{k} y_j\right)$, from .purple[Mendoza (2006)]
